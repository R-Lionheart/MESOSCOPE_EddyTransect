...
# Here is where we would hypothetically remove troublesome compounds, like Guanosine Monophosphate.

# Identify internal standards without an Area, i.e. any NA values.
IS_Issues <- HILIC.IS.data[is.na(HILIC.IS.data$Area.with.QC),]
...
HILIC.binded <- HILIC.long %>% select(-Metabolite.name) %>%
  rbind(HILIC.IS.data) %>%
